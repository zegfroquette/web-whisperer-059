...
import { supabase } from '@/integrations/supabase/next-client';
...
import {
  ...
  Phone, MapPin, Clock, Package, Settings2, FileText, ShieldCheck, PartyPopper, Pencil,
} from 'lucide-react';
...
type ServiceQuantities = { standardBags: number; maxBags: number; pieces: number };
type CountryCode = { code: string; name: string; dial: string; flag: string };
...
type FormData = {
  ...
  firstName: string; lastName: string; countryCode: string; phone: string;
  email: string; nif: string; preferredContact: 'call' | 'whatsapp' | '';
  pickupAddress: string; pickupPostcode: string; pickupFloor: string; pickupInstructions: string;
  pickupDate: Date | undefined; pickupSlot: string;
  sameDeliveryAddress: boolean;
  deliveryAddress: string; deliveryPostcode: string; deliveryFloor: string; deliveryInstructions: string;
  deliveryDate: Date | undefined; deliverySlot: string;
  washFold: ServiceQuantities; washIron: { pieces: number };
  ironingOnly: { pieces: number }; dryCleaning: { pieces: number };
  othersText: string; serviceNotes: string;
  antiAllergic: boolean; contactBeforeProceed: boolean; notes: string; consent: boolean;
};
...
const initial: FormData = {
  ...
  washIron: { pieces: 0 }, ironingOnly: { pieces: 0 }, dryCleaning: { pieces: 0 },
  othersText: '', serviceNotes: '', antiAllergic: false, contactBeforeProceed: false,
  notes: '', consent: false,
};
  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={() => onChange(Math.max(0, value - 1))} className="h-8 w-8 rounded-full border border-border flex items-center justify-center text-muted-foreground hover:bg-muted transition-colors">
          <Minus className="w-3.5 h-3.5" />
        </button>
        <span className="w-6 text-center font-semibold text-sm">{value}</span>
        <button type="button" onClick={() => onChange(value + 1)} className="h-8 w-8 rounded-full border border-primary/40 bg-primary/10 flex items-center justify-center text-primary hover:bg-primary/20 transition-colors">
...
function OptionButton({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} className={cn('flex-1 px-6 py-4 rounded-xl border-2 text-sm font-medium transition-all', selected ? 'border-primary bg-primary/10 text-primary shadow-sm' : 'border-border bg-card text-muted-foreground hover:border-primary/30')}>
      {children}
    </button>
  );
}
...
// ── Main component ──
export default function BookingContent() { return <BookingForm />; }
...
function BookingForm() {
  ...
  const b = (key: string) => {
    const map: Record<string, { pt: string; en: string }> = {
      ...
      consentText: { pt: 'Compreendo que a recolha e entrega estão disponíveis no concelho de Lisboa, sujeitas à capacidade, e que o preço final pode depender dos artigos recebidos. O pagamento é feito por cartão ou dinheiro na entrega.', en: 'I understand that pickup and delivery are available within municipal Lisbon, subject to item capacity, and that final pricing may depend on the items received. Payment is made by card or cash at delivery.' },
  ...
  const hasWashServices = () => (form.washFold.standardBags + form.washFold.maxBags + form.washFold.pieces + form.washIron.pieces) > 0;

  const validate = (): boolean => {
    const e: Record<string, string> = {};
    const req = b('required');
    if (step === 0) { if (form.returningCustomer === null) e.returningCustomer = req; }
    if (step === 1) {
    ...
    if (step === 2) {
      const has = form.washFold.standardBags + form.washFold.maxBags + form.washFold.pieces + form.washIron.pieces + form.ironingOnly.pieces + form.dryCleaning.pieces + (form.othersText.trim() ? 1 : 0);
    ...
    if (step === 7) { if (!form.consent) e.consent = b('acceptConsent'); }
    setErrors(e);
    return Object.keys(e).length === 0;
  };

  const goNext = () => { if (!validate()) return; setDirection(1); setStep(s => Math.min(s + 1, TOTAL_STEPS - 1)); };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const servicesPayload = { washFold: form.washFold, washIron: form.washIron, ironingOnly: form.ironingOnly, dryCleaning: form.dryCleaning, othersText: form.othersText, serviceNotes: form.serviceNotes };
      ...
      try {
        await supabase.functions.invoke('send-booking-notification', {
          body: {
            ...
            phone: `${form.countryCode} ${form.phone.trim()}`, preferredContact: form.preferredContact,
            returningCustomer: form.returningCustomer, pickupAddress: form.pickupAddress.trim(),
            pickupPostcode: form.pickupPostcode.trim(), pickupFloor: form.pickupFloor.trim() || undefined,
            pickupDate: format(form.pickupDate!, 'yyyy-MM-dd'), pickupSlot: form.pickupSlot,
            ...
            deliveryDate: format(form.deliveryDate!, 'yyyy-MM-dd'), deliverySlot: form.deliverySlot,
            services: servicesPayload, antiAllergic: form.antiAllergic, contactBeforeProceed: form.contactBeforeProceed,
            ...
            notes: form.notes.trim() || undefined, nif: form.nif?.trim() || undefined,
          },
        });
      } catch (emailErr) {
  ...
  const renderStep = () => {
    switch (step) {
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              <OptionButton selected={form.returningCustomer === true} onClick={() => { set('returningCustomer', true); clearError('returningCustomer'); setTimeout(() => { setDirection(1); setStep(1); }, 350); }}>{b('yes')}</OptionButton>
              <OptionButton selected={form.returningCustomer === false} onClick={() => { set('returningCustomer', false); clearError('returningCustomer'); setTimeout(() => { setDirection(1); setStep(1); }, 350); }}>{b('no')}</OptionButton>
            </div>
          ...
          <div className="space-y-5">
              ...
              <div className="flex gap-2">
                <Select value={form.countryCode} onValueChange={v => set('countryCode', v)}>
                  <SelectTrigger className="w-[110px] shrink-0">
                    <SelectValue>{COUNTRY_CODES.find(c => c.dial === form.countryCode)?.flag} {form.countryCode}</SelectValue>
                  </SelectTrigger>
                  <SelectContent className="max-h-[280px]">
                    {COUNTRY_CODES.map(c => (
                      <SelectItem key={c.code} value={c.dial}>
                        <span className="flex items-center gap-2"><span>{c.flag}</span><span>{c.dial}</span><span className="text-muted-foreground text-xs">{c.name}</span></span>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <Input type="tel" value={form.phone} onChange={e => { set('phone', e.target.value); clearError('phone'); }} className="flex-1" placeholder="912 345 678" />
              </div>
            ...
            <div>
              ...
              <div className="flex gap-3 mt-2">
                <OptionButton selected={form.preferredContact === 'call'} onClick={() => { set('preferredContact', 'call'); clearError('preferredContact'); }}>{b('call')}</OptionButton>
                <OptionButton selected={form.preferredContact === 'whatsapp'} onClick={() => { set('preferredContact', 'whatsapp'); clearError('preferredContact'); }}>{b('whatsapp')}</OptionButton>
              </div>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-border/50 bg-card p-4 space-y-2">
              ...
              <Textarea value={form.serviceNotes} onChange={e => set('serviceNotes', e.target.value)} placeholder={language === 'pt' ? 'ex: as peças para engomar são as camisas e t-shirts dentro da Bolsa Max' : 'e.g. the items for ironing are the shirts and t-shirts inside the Max Bag'} className="min-h-[80px]" />
            </div>
          </div>
        );
      ...
      case 3: {
        ...
        if (!showAntiAllergic && form.antiAllergic) set('antiAllergic', false);
        return (
          <div className="space-y-6">
          ...
          <div className="space-y-5">
            ...
            {hasDryCleaning() && (
              <div className="inline-flex items-center gap-2 rounded-lg bg-muted/60 border border-border/50 px-4 py-2.5 text-xs text-muted-foreground">
                <Clock className="w-4 h-4 text-primary shrink-0" /><span>{b('dryCleaningNote')}</span>
              </div>
            )}
            <div>
              <Label>{b('sameAddress')}</Label>
              <div className="flex gap-3 mt-2">
                <OptionButton selected={form.sameDeliveryAddress} onClick={() => set('sameDeliveryAddress', true)}>{b('yes')}</OptionButton>
                <OptionButton selected={!form.sameDeliveryAddress} onClick={() => set('sameDeliveryAddress', false)}>{b('no')}</OptionButton>
              </div>
            </div>
      ...
      case 6:
        return (
          <div className="space-y-5">
            ...
            <Textarea value={form.notes} onChange={e => set('notes', e.target.value)} placeholder={b('notesPlaceholder')} className="min-h-[140px]" />
          </div>
        );
          ...
          <div className="space-y-4">
            ...
            <Section title={b('stepDelivery')} stepIdx={5}>
              {form.sameDeliveryAddress ? (
                ...
                <><p>{form.deliveryAddress}, {form.deliveryPostcode}</p>{form.deliveryFloor && <p>{form.deliveryFloor}</p>}</>
              )}
              <p>{form.deliveryDate ? format(form.deliveryDate, 'PPP') : ''} · {form.deliverySlot}</p>
            </Section>
            {form.notes.trim() && (
              <Section title={b('stepNotes')} stepIdx={6}><p>{form.notes}</p></Section>
            )}
            <div className="pt-2">
              <div className="flex items-start gap-3">
                <Checkbox id="consent" checked={form.consent} onCheckedChange={v => { set('consent', v as boolean); clearError('consent'); }} className="mt-1" />
                <label htmlFor="consent" className="text-sm text-muted-foreground leading-relaxed cursor-pointer">{b('consentText')}</label>
              </div>
        ...
        return (
          <div className="text-center space-y-6 py-8">
            ...
            <p className="text-muted-foreground max-w-md mx-auto">
              {b('confirmationMsg')}{' '}
              <strong>{form.pickupDate ? (language === 'pt' ? form.pickupDate.toLocaleDateString('pt-PT', { day: 'numeric', month: 'long', year: 'numeric' }) : format(form.pickupDate, 'PPP')) : ''}</strong>
              {' '}{b('between')} <strong>{form.pickupSlot}</strong>.
            </p>
            <p className="text-sm text-muted-foreground">{b('confirmationNote')}</p>
            <Button onClick={() => { setForm({ ...initial }); setStep(0); }} variant="outline" className="mt-4">{b('bookAnother')}</Button>
          </div>
        );
    ...
    <div className="min-h-[80vh] py-12 px-4">
      <div className="max-w-xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          <h1 className="text-3xl sm:text-4xl font-bold mb-3" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>{b('pageTitle')}</h1>
          ...
          <div className="mt-4 inline-flex items-center gap-2 rounded-lg bg-muted/60 border border-border/50 px-4 py-2.5 text-xs text-muted-foreground">
            <ShieldCheck className="w-4 h-4 text-primary shrink-0" /><span>{b('paymentInfo')}</span>
          </div>
        </motion.div>
        ...
        {step < 8 && (
          <div className="mb-8">
              ...
              {Array.from({ length: TOTAL_STEPS - 1 }).map((_, i) => {
                ...
                return (
                  <React.Fragment key={i}>
                    {i > 0 && (<div className={cn('flex-1 h-0.5 transition-colors duration-300', isCompleted ? 'bg-primary' : 'bg-border')} />)}
                    <div className="flex flex-col items-center">
                      <div className={cn('w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 border-2', isCompleted && 'bg-primary border-primary text-primary-foreground', isCurrent && 'border-primary bg-primary/10 text-primary shadow-md shadow-primary/20', !isCompleted && !isCurrent && 'border-border bg-muted/50 text-muted-foreground')}>
                        {isCompleted ? <Check className="w-4 h-4" /> : <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                      </div>
                      <span className={cn('text-[10px] sm:text-xs text-center leading-tight mt-1.5 hidden sm:block', isCurrent ? 'text-primary font-semibold' : isCompleted ? 'text-foreground/70' : 'text-muted-foreground')}>
                        {stepTitles[i]}
                      </span>
                    </div>
                  </React.Fragment>
            ...
            <p className="text-center text-xs text-primary font-medium mt-3 sm:hidden">{step + 1}/{TOTAL_STEPS - 1} — {stepTitles[step]}</p>
          </div>
        )}

        <AnimatePresence mode="wait" custom={direction}>
          <motion.div key={step} custom={direction} initial={{ opacity: 0, x: direction * 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: direction * -40 }} transition={{ duration: 0.25 }} className="bg-card rounded-2xl border border-border/50 shadow-sm p-6 sm:p-8">
            {renderStep()}
          </motion.div>
        </AnimatePresence>
        ...
        {step < 8 && (
          <div className="flex items-center justify-between mt-6">
            {step > 0 ? (
              <Button variant="ghost" onClick={goBack} className="gap-1"><ChevronLeft className="w-4 h-4" /> {b('back')}</Button>
            ) : <div />}
            {step < 7 ? (
              <Button onClick={goNext} className="gradient-primary text-primary-foreground gap-1 px-6">{b('next')} <ChevronRight className="w-4 h-4" /></Button>
            ) : step === 7 ? (
              <Button onClick={handleSubmit} disabled={submitting} className="gradient-primary text-primary-foreground px-8">{submitting ? '...' : b('submit')}</Button>
            ) : null}
          </div>
        )}
      </div>
